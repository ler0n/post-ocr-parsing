...
def isnum(input, input_original, catbef):
    ...
    tel_list = ['02', '051', '053', '032', '062', '042', '052', '044', '031', '033', '043', '041', '063', '061', '054',
                '055', '064', '070']
    n_tel_list = []
    for tel in tel_list:
        temp = tel[1:]
    ...
    if language == "한국어":
        for position in position_list:
            if position.lower() == input_original[-len(position):].lower():
                ans, alpha_temp, height = isname(input_original[:-len(position)], input_original[:-len(position)],
                                                 language, last_name_list, last_name_list_en, first_name_list, height,
                                                 position_list)
                if ans == 0:
...
def isemail(input_original):
    ...
    email = ['@gmail.com', '@outlook.kr', '@outlook.com', '@hotmail.com', '@icloud.com', '@mac.com', '@me.com',
             '@naver.com', '@hanmail.net',
             '@daum.net', '@nate.com', '@kakao.com', '@citizen.seoul.kr', '@yahoo.co.jp', '@yahoo.com', '@yandex.com',
             '@yandex.ru']
    for i in elist_temp:
        elist.append(i + ".")
        elist.append(i + ":")
...
def mainfun(serialized):
    ...
    for i in range(len(inputlist)):
        ...
        alpha_temp = 1
        # position
        postion_candidate, name_candidate = isposition(inputlist[i], position_list, language, last_name_list,
                                                       last_name_list_en, first_name_list, height)
        ...
        if postion_candidate == 0 and email_candidate == 0:
            while alpha_temp > 0 or chalpha == 0:
                ...
                if input_changed[i][alpha:] != '':
                    ans, alpha_temp, height = isname(input_changed[i][alpha:], inputlist[i][alpha:], language,
                                                     last_name_list, last_name_list_en, first_name_list, height,
                                                     position_list)
